    ...
    <View className="gap-2.5">
      ...
      {routes.map((route) => (
        <Pressable key={route.id} onPress={() => onSelect(route)} className="flex-row items-center gap-3 active:opacity-75" style={{ borderRadius: Radius.lg, backgroundColor: theme.backgroundElement, borderWidth: 1, borderColor: theme.border, padding: 13 }}>
          ...
          <View className="items-end"><ThemedText style={{ fontSize: 14, fontWeight: '900', color: Semantic.positive, ...MONO }}>+${route.expectedReturn}</ThemedText><ThemedText style={{ fontSize: 11, color: theme.textSecondary, ...MONO }}>{route.probability}% chance</ThemedText></View>
        </Pressable>
      ))}
    </View>
  );

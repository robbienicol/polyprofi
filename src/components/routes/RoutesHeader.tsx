      ...
      <View className="flex-row justify-between items-start">
        <View className="gap-0.5 flex-1">
          ...
          <View className="flex-row items-baseline gap-1.5">
            <ThemedText style={{ fontSize: 30, fontWeight: '800', color: theme.text, letterSpacing: -0.8, fontVariant: ['tabular-nums'] }}>Make +${goal.target}</ThemedText>
            <ThemedText style={{ fontSize: 13, color: theme.textTertiary }}>{goal.when}</ThemedText>
          </View>
        </View>
        {!historical && (
          <Pressable onPress={onNewSearch} className="active:opacity-60" style={{ borderRadius: Radius.md, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: theme.backgroundSelected }}>

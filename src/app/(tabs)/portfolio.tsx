...
export default function PortfolioScreen(): React.ReactElement {
  ...
  // Only claim a target we actually know: the quiz's goal, else the savings goal
  // on top of what's staked. With neither, don't invent one from our own output.
  const targetValue = latestSearch
    ? latestSearch.quizSnapshot.balance + latestSearch.quizSnapshot.target
    : goal
      ? staked + goal.targetAmount
      : null;
    ...
    <View className="flex-1" style={{ backgroundColor: theme.background }}>
      <SafeAreaView className="flex-1">
        <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="px-4 pt-4 pb-16 gap-4">
          ...
          {isEmpty ? (
            <EmptyPortfolio onFindRoutes={() => router.push('/(tabs)/routes')} />
          ) : (
            <>
              ...
              {activeBets.length === 0 ? (
                ...
                <View style={{ gap: 10 }}>
                  <View className="flex-row items-center justify-between" style={{ paddingHorizontal: 6 }}>
                    ...
                    <Pressable onPress={() => router.push('/positions')} className="active:opacity-60">
                  ...
                  {activeBets.length > 5 ? (
                    <Pressable onPress={() => router.push('/positions')} className="items-center py-2 active:opacity-60">
...
function EmptyPortfolio({ onFindRoutes }: { onFindRoutes: () => void }): React.ReactElement {
  const theme = useTheme();
  return (
    <View
      ...
      <ThemedText style={{ fontSize: 17, fontWeight: '800', color: theme.text }}>No portfolio yet</ThemedText>
      <ThemedText className="text-center" style={{ fontSize: 13, lineHeight: 19, color: theme.textSecondary, maxWidth: 260 }}>
        Set a goal, pick a route, and your allocation, projected value, and odds of hitting the target all show up here.
      </ThemedText>
      <Pressable
